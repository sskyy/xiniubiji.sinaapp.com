...
class Table{
    ...
    private function _standardize_condition_arguments( $arguments = false ){
        ...
        if( count( array_intersect( $this -> condition_keys, array_keys( $arguments ) ) ) ) {
            $output = $arguments;
        }else{
            foreach( $arguments as $key => $argument ){
                if( is_array( $argument )){
                    $output['where_in'][$key] =  $argument ;
                }elseif( is_string( $argument ) || is_numeric( $argument ) ){
                    $output['where'][$key] = $argument;
                }
            }
        }
        return $output;
    ...
    private function _db_set_conditions(){
        ...
        foreach( $this -> condition_keys as $condition_key ){
            if( isset( $conditions[$condition_key] ) ){
                foreach( $conditions[$condition_key] as $data_key => $data_value ){
                    $this -> db -> $condition_key( $data_key, $data_value );
                }
            }
        }
    }
