...
(function($){
	...
	$.fn.portamento = function(options) {
		...
		var fullyCapableBrowser = positionFixedSupported();
                                    var container_id = opts.container_id ? opts.container_id : 'portamento_container';
		...
		panel.wrap('<div id="'+container_id+'" />');
		var float_container = $('#'+container_id);
		float_container.css({
			'min-height': panel.outerHeight(),
			'width': panel.outerWidth()
		});
                console.log("getScrollerWidth() ", getScrollerWidth()  );
		
                
                                    function calculateBoundary( panel, gap ){
                                        // calculate the upper scrolling boundary
                                        var panelOffset = panel.offset().top;
                                        var panelMargin = parseFloat(panel.css('marginTop').replace(/auto/, 0));
                                        var realPanelOffset = panelOffset - panelMargin;
                                        var topScrollBoundary = realPanelOffset - gap;
                                        return topScrollBoundary;
                                    }
                                    
		var topScrollBoundary = calculateBoundary( panel, gap );
